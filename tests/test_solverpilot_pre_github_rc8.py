...
def test_rc8_public_api_snapshot_exactly_matches_runtime():
    payload = json.loads((ROOT / "PUBLIC-API-V0_3_0.json").read_text())
...
def test_rc8_backend_contract_matches_runtime_and_learning_stays_off():
    payload = json.loads((ROOT / "BACKEND-CONTRACT-V0_3_0.json").read_text())
...
def test_rc8_current_manifest_requires_rc8_contracts_not_rc6():
    ...
    assert "include PUBLIC-API-V0_3_0.json" in manifest
    assert "include BACKEND-CONTRACT-V0_3_0.json" in manifest
