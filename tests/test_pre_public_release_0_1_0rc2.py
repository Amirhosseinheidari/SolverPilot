...
def test_final_release_identity_is_distinct_from_historical_rc2():
    ...
    assert project["version"] == solverpilot.__version__ == "0.4"
...
def test_current_release_snapshots_are_versioned_without_label_collision():
    api = json.loads((ROOT / "PUBLIC-API-V0_4_0.json").read_text(encoding="utf-8"))
    backend = json.loads((ROOT / "BACKEND-CONTRACT-V0_4_0.json").read_text(encoding="utf-8"))
    assert api["package_version"] == "0.4"
    assert backend["package_version"] == "0.4"
